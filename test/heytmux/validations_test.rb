...
class HeytmuxValidationsTest < HeytmuxTestBase
  ...
  def test_validate_command
    ['', 'foo', ['foo', 'bar', 'expect' => 'pattern']].each do |spec|
      assert_nil Heytmux::Validations.validate_commands(spec)
    end
    [nil, [nil], ['not expected' => 'pattern']].each do |spec|
      assert_raises(ArgumentError) do
...
class HeytmuxTest < HeytmuxTestBase
  ...
  def test_process!
    spec = {
      'windows' => [
        'heytmux-test-1',
        { 'heytmux-test-2' =>
          { 'panes' => [{ 'heytmux-test2-1' => 'echo -n "foo"; sleep 60' },
                        { 'heytmux-test2-2' => 'echo -n "bar"; sleep 60' }] } }
      ]
    }
    Heytmux.process!(spec, false)
    list = Tmux.list
    ...
    (panes1 + panes2).map { |p| p[:window_index] }
                     .uniq.sort.reverse.each do |index|
      Tmux.kill(index)
    end
  end
end
